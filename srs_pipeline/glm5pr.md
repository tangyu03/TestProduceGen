...
- **聚合/记录成对动作前置一致（铁律）**：同一业务动作既落记录级（子项，如报名记录）又落聚合级（父容器，如项目）状态面时，聚合级前置＝自身状态值；记录级结果性门禁（E-ID.状态值），不得只写自身状态值。例：编制结果报告/编制结果通知单——记录级 E-BMJL（结果已提交→报告/证书审核中）前置为"结果已提交"，聚合级 E-XM.项目状态（进行中→报告审核中）前置必须为"进行中；E-BMJL.结果已提交"；漏写 → 跨主体门禁缺失，相位对齐与依赖连边失败，报告编制类用例会错误排到结果提交、评价之前。
...
- 聚合级镜像记录级同动作转换时，须携带与记录级一致的跨主体 state_ref 门禁（例：项目级"编制结果报告/编制结果通知单"前置须含 E-BMJL.报名记录状态.结果已提交），仅写自身状态值视为漏门禁。
