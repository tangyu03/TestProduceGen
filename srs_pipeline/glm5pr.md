...
**聚合/记录成对动作前置一致（铁律）**：同一动作既落记录级（明细/子记录实体）又落聚合级（父容器实体）状态面时，聚合级行前置＝「自身状态值；记录实体.记录行前置值」，不得只写自身状态值。漏写 → 跨主体门禁缺失，相位对齐与依赖连边失败，该动作的用例会错误排到记录级前置动作之前。例：编制结果报告——记录级 E-BMJL（结果已提交→报告/证书审核中）前置「结果已提交」，聚合级 E-XM.项目状态（进行中→报告审核中）前置须为「进行中；E-BMJL.结果已提交」。
...
- 同动作聚合/记录成对（§2 铁律）：聚合级行须携与记录行一致的跨主体 state_ref 门禁，仅写自身状态值＝漏门禁。
