...
8. 如果 Given 里有分支条件（如 "分支条件: 任务级别=C级"），标题必须体现该分支的实际取值（C级/B级/A级），不得沿用其他分支的取值（曾致 C 级分支继承 B 级标题）；无分支条件时不要凭空添加级别字样
...
输入步骤（带分支条件，标题须用该分支取值）：
  GIVEN: E-IMP.任务状态 = 待审批 (导出任务处于待审批状态)
  GIVEN: (任务级别=C级) 分支条件: 任务级别=C级
  WHEN: E-IMP.任务状态 审批通过事件触发 by 一级审批员
  THEN: E-IMP.任务状态 状态转换为审批通过 (state)
输出：C级导出任务待审批时，一级审批员审批通过，验证任务状态变更为审批通过"""
