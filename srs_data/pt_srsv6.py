...
from srs_pipeline import DomainModel, attr, op, precond, state_ref
...
def build() -> DomainModel:
    ...
    m.add_event("e01", entity="E-XM", dimension="项目状态", action="设计方案编制",
                actor="策划人员", precondition="待开始", consequence="待开始",
    ...
    m.add_event("e37", entity="E-STD", dimension="标准库状态", action="新增标准库",
                actor="系统管理人员",
                precondition="启用", consequence="启用",
        ...
        state_dimensions=[
            {
                ...
                "terminal": ["已过期"],
                "inferred": ["即将到期", "已过期"],
                "note": {"comment": "状态枚举文档未显式列出，按 §20.5.2.3/§20.6.2.3 证书到期前30天提醒的语义推导 inferred：距到期=30天进入 即将到期，过期则 已过期"},
            },
        ],
        operations=[
    ...
    m.add_branch_dimension(
        ...
        branches=[
            {"value": "分值", "target_transition": "t41"},
            {"value": "权重", "target_transition": "t41"},
        ],
    )
    ...
    m.add_branch_dimension(
        ...
        branches=[
            ...
            {"value": "测量审核", "target_transition": "t02b"},
        ],
    )
    # ② 运行时选择型：报名审核结果
    m.add_branch_dimension(
        ...
        branches=[
            {"value": "通过", "target_transition": "t08"},
            {"value": "退回修改", "target_transition": "t08b"},
        ],
    )
    # ② 运行时选择型：参加者样品归还
    m.add_branch_dimension(
        ...
        values=["已还样", "无需还样"],
        impact_scope="影响E-YP样品状态落点",
        evidence="②运行时选择型：§19.1实施阶段-参加者测试与结果提交行 样品状态列 已还样、待核查/无需还样",
        branches=[
            {"value": "已还样", "target_transition": "t19"},
            {"value": "无需还样", "target_transition": "t19b"},
        ],
    )
    # ② 运行时选择型：预通知发送确认
    m.add_branch_dimension(
        dimension="预通知确认结果", entity="E-YT",
        values=["已发送", "待确认"],
        impact_scope="影响E-YT预通知状态落点",
        evidence="②运行时选择型：§19.1实施阶段-能力验证预通知行 预通知状态列 已发送/待确认",
        branches=[
            {"value": "已发送", "target_transition": "t23"},
            {"value": "待确认", "target_transition": "t23b"},
        ],
    )
    # ② 运行时选择型：实验室审核结果
    m.add_branch_dimension(
        ...
        branches=[
            {"value": "通过", "target_transition": "t33"},
            {"value": "退回修改", "target_transition": "t33b"},
        ],
    )
    # ② 运行时选择型：审核任务结果
    m.add_branch_dimension(
        ...
        branches=[
            {"value": "同意", "target_transition": "t45"},
            {"value": "退回", "target_transition": "t46"},
        ],
    )
    ...
    m.add_trans(
        ...
        traits=["branch"], direction="forward", priority="P0",
        source_ref="19.1实施阶段-参加者测试与结果提交行",
        note={"branch_dimension": "样品归还方式", "comment": "源自 e20；③frm先于to→forward；路径分歧：已还样分支"},
    )
    m.add_trans(
        ...
        source_ref="19.1实施阶段-参加者测试与结果提交行",
        note={"branch_dimension": "样品归还方式", "inferred": True, "comment": "源自 e20b；③frm先于to→forward；inferred 状态 无需还样 由 §19.1 推导"},
    )
    ...
    m.add_trans(
        ...
        traits=["branch"], direction="forward", priority="P0",
        source_ref="19.1实施阶段-能力验证预通知行",
        note={"branch_dimension": "预通知确认结果", "comment": "源自 e23；③frm先于to→forward；路径分歧：已发送分支"},
    )
    m.add_trans(
        ...
        traits=["branch"], direction="forward", priority="P1",
        source_ref="19.1实施阶段-能力验证预通知行",
        note={"branch_dimension": "预通知确认结果", "comment": "源自 e24；③frm先于to→forward；路径分歧：待确认分支"},
    )
    m.add_trans(
    ...
    # i06 已删除：删除测试项是操作禁止（E-CS 无状态维度，frm/to 无法填合法状态值），
    # 非 invalid 规格（glm5pr §5：仅明文禁止的状态转换）适用场景；删除门禁已由
    # b10（constrained_entity=E-CS）与 E-CS.删除测试项 operation 的 expected_results 承载
    ...
    m.add_br(
        ...
        note={"comment": "signal_type 命中'提示信息...确认后...执行'；category 判 usability；多实体对称规则"},
    )
    m.add_br(
        ...
        enforcement="mandatory",
        note={"role": "r08", "comment": "signal_type 命中'大于等于...小于'；category 判 computation；constrained_entity=E-PJ",
    ...
    m.add_br(
        ...
        restrictive=True,
        note={"comment": "signal_type 命中'不可篡改'；category 判 validation；多实体 BR"},
    )
    m.add_br(
