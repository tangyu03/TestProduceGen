...
def build() -> DomainModel:
    ...
    m.add_entity(
        ...
        operations=[
            op("新增项目", "crud", ["项目创建并进入状态流转；能力验证分支经设计方案编制后初始状态为待开始，测量审核分支受理报名后初始状态为报名中"], "20.5.1.5项目新增表单增加监督员",
               note=N(role=["项目管理员"], comment="点击菜单或项目列表页'新增'进入项目新增表单页面；项目状态创建落点由台账e01设计方案编制承载；两种项目类型的创建落点不同（§19.2 能力验证/测量审核平行流程）")),
            ...
            op("文件整理", "file", ["系统开启整理任务并提示用户'归档任务已开启，请稍后查看'", "整理完成后操作列会显示【查看归档】按钮"], "20.5.1.1文件整理",
               ...
               stage_hint={"anchor_state": {"entity": "E-XM", "dimension": "项目状态", "state": "已结束"}}),
            ...
            op("上传结果通知单", "file", ["结果通知单上传成功，可批量提交审核（推断）"], "20.5.1.3项目批量操作",
               note=N(role=["项目管理员"], comment="category①file；按处理内容与是否已上传判定按钮显示；报告/证书阶段批量处理页操作"),
               stage_hint={"anchor_state": {"entity": "E-XM", "dimension": "项目状态", "state": "报告审核中"}}),
    ...
    m.add_entity(
        ...
        operations=[
            ...
            op("新增测试项", "crud", ["点击提交表单，新增测试项"], "20.4.2.8新增测试项",
               ...
               form_fields=[{"name": "标号", "desc": "测试项编号"}, {"name": "名称", "desc": "测试项名称"}]),
    ...
    m.add_trans(
        ...
        traits=[], direction="forward", priority="P0",
        source_ref="19.1实施阶段；19.4能力验证参加者工作流程分析",
        note={"branch_dimension": "项目类型", "comment": "源自e19；③；能力验证分支（action/role/来源19.4均能力验证专属），落点值取19.1/19.3样品发放行预通知状态列'已确认'，动作短语取19.4参加者流程；§5修复C32：随维度迁E-XM（项目级预通知状态面）"},
        branch_values=["能力验证"],
    )
    m.add_trans(
        ...
        note={"branch_dimension": "项目类型", "comment": "源自e20；③；能力验证分支（source_ref仅19.1流程表，无19.2证据，测量审核侧无独立样品发放转换），跨维度门禁：19.1样品发放行预通知状态列'已确认'（通知状态）落state_ref；歧义：原文列值'已发样'与枚举值'待收样'同节点两口径并列取枚举"},
        branch_values=["能力验证"],
    )
    m.add_trans(
    ...
    m.add_trans(
        ...
        traits=["branch"], direction="forward", priority="P0",
        source_ref="19.3项目状态分析",
        note={"branch_dimension": "项目类型", "comment": "源自e40；⓪创建转换；测量审核分支（先受理报名后立项，与t01分立），Step2 value=测量审核→本条；预通知状态出生点=项目创建（19.3测量审核流无'能力验证计划发布'宿主动作，镜像t07能力验证面的初始化声明），作业指导书编制（t46）前置'未发送'由本条建立"},
        branch_values=["测量审核"],
    )
    m.add_trans(
    ...
    m.add_trans(
        ...
        preconditions=[],
        expected_results=["点击提交表单，创建标准库，状态为表单所选值（启用/停用）"],
        traits=["form_selected_state"], direction="forward", priority="P1",
        source_ref="20.4.2.2新增标准库",
        note={"comment": "源自e58；⓪创建转换；表单状态单选含启用/停用（必填），缺省取启用，停用创建为表单分支差异经属性承载；form_selected_state=创建转换的目标状态由表单所选值决定，S1 不合成'状态转换为{to}'硬断言"},
    )
    m.add_trans(
    ...
    m.add_trans(
        ...
        traits=[], direction="forward", priority="P0",
        source_ref="19.4能力验证参加者工作流程分析",
        note={"branch_dimension": "项目类型", "inferred": True, "comment": "源自e68；③；能力验证分支（role/来源19.4均能力验证专属），19.4参加者流程'接收样品：参加者接收样品'，19.3枚举含'已收样'，补链"},
        branch_values=["能力验证"],
    )
    m.add_trans(
        ...
        traits=[], direction="forward", priority="P1",
        source_ref="19.3项目状态分析",
        note={"branch_dimension": "项目类型", "inferred": True, "comment": "源自e69；③；能力验证分支（role=能力验证参加者），19.3枚举'已确认'为报名记录样品状态终值，与预通知确认机制对齐，收样确认补链"},
        branch_values=["能力验证"],
    )
