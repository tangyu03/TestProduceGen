...
def build() -> DomainModel:
    ...
    m.add_event("e07c", "E-JK", "缴费状态", "缴费", "能力验证参加者",
                ...
                note={"comment": "创建事件 inferred：20.10.2.3 缴费记录实体存在但创建流程缺失"})
    ...
    m.add_event("e20b", "E-BM", "通知状态", "样品领用登记", "样品管理员",
                "未发送", "待审核", "19.2实施阶段",
                note={"comment": "19.2流程表'样品领用登记'行预通知状态列由未发送变待审核"})
    m.add_event("e21", "E-BM", "通知状态", "作业指导书编制", "策划人员",
                "待审核", "已审核", "19.2实施阶段")
    m.add_event("e21b", "E-BM", "通知状态", "作业指导书退回", "策划人员",
                "待审核", "退回", "19.2实施阶段",
                note={"comment": "19.2流程表'作业指导书编制'行预通知状态列取值'待审核/退回/已审核'"})
    m.add_event("e21c", "E-BM", "通知状态", "作业指导书重新提交", "策划人员",
                "退回", "待审核", "19.2实施阶段",
                note={"comment": "inferred：退回后修改重新提交，状态机闭环"})
    m.add_event("e21d", "E-BM", "通知状态", "能力验证预通知", "项目管理员",
                "已审核", "已发送", "19.2实施阶段",
                note={"comment": "19.2流程表'能力验证预通知'行预通知状态列由已审核变已发送/待确认"})
    # ── 20.4.1 实验室管理 ──
