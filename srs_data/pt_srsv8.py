...
def build() -> DomainModel:
    ...
    m.add_br(bid="b09", category="computation", desc="评价支持分值和权重两种方式，分值按累加计算得分，权重按加权计算得分", entities_involved=["E-PJ"], source_ref="20.7.1项目列表", note={"role": ["评价人员"], "comment": "signal_type命中两种(取值范围)；category判计算衍生"}, branch_dimensions=["评分方式"])
    ...
    m.add_br(bid="b11", category="validation", desc="含有子项的记录不允许删除，删除前系统做前置判断", entities_involved=["E-STD"], source_ref="20.4.2.10删除测试项；20.4.3.4删除测试项", restrictive=True, note={"comment": "signal_type命中不允许(禁止)；category判校验"}, constrained_entity="E-STD")
    ...
    m.add_br(bid="b16", category="authorization", desc="机构新增或修改实验室信息后需经管理用户审核通过后方可用于项目报名", entities_involved=["E-LAB"], source_ref="20.3.1实验室信息", restrictive=True, note={"role": ["系统管理人员"], "comment": "signal_type命中后方可(条件限制)；category判授权"}, constrained_entity="E-LAB")
    ...
    m.add_br(bid="b20", category="authorization", desc="新建项目时第一个被选择的评价人员默认作为评价组长，评价组长可以在评价结果确认页面查看各评价人员的评价结果并对最终结果进行确认", entities_involved=["E-PJ"], source_ref="20.7.1项目列表", note={"role": ["评价人员"], "comment": "signal_type命中默认(规则)；category判授权"}, constrained_entity="E-PJ")
    ...
    m.add_br(bid="b22", category="authorization", desc="对关键操作实施留痕机制，系统自动记录操作者的身份、时间戳、操作细节及结果，生成不可篡改的审计日志", entities_involved=[], source_ref="20.11.1.2安全性相关内容优化", note={"comment": "signal_type命中自动记录(规则)；category判授权/审计"})
    m.add_br(bid="b23", category="usability", desc="项目新增表单中技术主管、实验室负责人、授权签字人字段，如果其备选人有且仅有一个时默认填充为备选值", entities_involved=["E-XM"], source_ref="20.5.1.6默认填充技术主管实验室负责人授权签字人", note={"comment": "signal_type命中有且仅有一个时默认填充(规则)；category判可用性"}, constrained_entity="E-XM")
    m.add_br(bid="b24", category="usability", desc="系统内增加电子签章位置信息，当进行签章操作时自动代入此位置信息减少手动调整操作", entities_involved=["E-TASK"], source_ref="20.9.1.2预置签章位置信息", note={"comment": "signal_type命中自动代入(规则)；category判可用性"}, constrained_entity="E-TASK")
    m.add_br(bid="b25", category="usability", desc="测量审核结果通知单审批流程合并为一个流程，流程处理人审批顺序为提交申请时签字人的选择顺序", entities_involved=["E-TASK"], source_ref="20.9.1.1测量审核结果通知单审核流程优化", note={"comment": "signal_type命中合并为/为(规则)；category判可用性"}, constrained_entity="E-TASK")
