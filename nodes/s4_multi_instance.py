...
def s4_multi_instance_node(state: AgentState) -> dict:
    ...
    # Fix-7: identify singleton entities — those with no state-machine dimensions
    # (e.g. E-LOG 系统日志, E-WEIGHT 打分权重配置, E-LEVEL 项目级别评判).
    # These are L0 managed/configuration entities with a single logical
    # instance — duplicating them provides no test value (all instances
    # would be textually identical).
    singleton_entities: set[str] = set()
    ctx = cm.get("_context", {}) if isinstance(cm, dict) else {}
    for ed in ctx.get("entity_details", []) or []:
        eid = ed.get("id", "") or ed.get("entity_id", "")
        tags = ed.get("tags", []) or []
        # Heuristic: entities tagged "configurable" without state-machine
        # transitions are singletons (config tables that take effect system-wide).
        has_state_machine = any(
            to.get("entity") == eid and to.get("dimension")
            for to in tos
        )
        if eid and not has_state_machine and "configurable" in tags:
            singleton_entities.add(eid)
    # Also mark E-LOG as singleton (system log — only one logical instance)
    for ed in ctx.get("entity_details", []) or []:
        eid = ed.get("id", "")
        ename = ed.get("name", "")
        if eid and ("日志" in ename or "log" in ename.lower()):
            singleton_entities.add(eid)
    ...
    # Fix-7: primary entity is never singleton (it has the main state machine)
    singleton_entities.discard(primary)
    ...
    for proc in procedures:
        ...
        if ot == 8:  # Type7 standalone: use BR.entities_involved[0]
            ...
            # Fix-7: singleton suppression for Type7's first_entity
            if first_entity in singleton_entities:
                count = 1
        else:
            count = entity_instances.get(entity, 1)
            # Fix-7: singleton suppression
            if entity in singleton_entities:
                count = 1
        ...
        s4["multi_reason"] = (
            ...
            + (" [singleton]" if count == 1 and entity in singleton_entities else "")
        )
        ...
        for instance_num in range(1, count + 1):
            ...
            if count > 1:
                ...
                # Fix-7: inject instance identifier into givens[0].description
                # so multi-instance procedures are textually distinguishable
                # (otherwise all instances have identical givens/when/thens,
                # making the duplication meaningless).
                if instance_proc.get("givens"):
                    g0 = instance_proc["givens"][0]
                    orig_desc = g0.get("description", "")
                    g0["description"] = f"[实例 {instance_num}] {orig_desc}".strip()
    ...
    # ── Dependency reference repair (v29 #20: paired, not fan-out) ──
    ...
    #
    # v28 behavior: replace original ID with ALL instance IDs (fan-out).
    #   PROC-073.1 deps: [PROC-071.1, PROC-071.2, PROC-071.3]
    #   PROC-073.2 deps: [PROC-071.1, PROC-071.2, PROC-071.3]
    # Problem: causes 17.6x dependency inflation, breaks paired semantics.
    #
    # v29 #20 behavior: pair by instance number (1:1).
    #   PROC-073.1 deps: [PROC-071.1]   # 1st 报名 ↔ 1st 项目
    #   PROC-073.2 deps: [PROC-071.2]   # 2nd 报名 ↔ 2nd 项目
    #   PROC-073.3 deps: [PROC-071.3]   # 3rd 报名 ↔ 3rd 项目
    # If dep has more instances than current, extras are dropped (current
    # instance only pairs with same-numbered dep instance).
    # If dep has fewer instances than current, the LAST dep instance is
    # reused (so instance 5 of a 3-instance proc still has a dep).
    ...
    if expanded_ids_by_base:
        for p in expanded:
            s3 = p.get("_S3_fields", {})
            # Determine this proc's instance number (1-based)
            tid = p["temp_id"]
            if "." in tid:
                _, inst_str = tid.rsplit(".", 1)
                try:
                    my_inst_num = int(inst_str)
                except ValueError:
                    my_inst_num = 1
            else:
                my_inst_num = 1  # single-instance proc
            ...
            for field in ("dependencies", "weak_dependencies"):
                ...
                for d in deps:
                    if d in expanded_ids_by_base:
                        # v29 #20: pair by instance number
                        inst_ids = expanded_ids_by_base[d]
                        if my_inst_num <= len(inst_ids):
                            # 1:1 pairing — pick the same-numbered instance
                            paired_id = inst_ids[my_inst_num - 1]
                        else:
                            # Current has more instances than dep —
                            # reuse the LAST dep instance (better than
                            # dropping the dep entirely)
                            paired_id = inst_ids[-1]
                        if paired_id != p["temp_id"] and paired_id not in seen:
                            new_deps.append(paired_id)
                            seen.add(paired_id)
                    elif d not in seen and d != p["temp_id"]:
                        # Single-instance dep, keep as-is
                        new_deps.append(d)
                        seen.add(d)
                s3[field] = new_deps

            # Fix-1b: 同步重写 dep_origins / weak_origins 字典的 key
            # 这些字典记录每个 dep 的来源（transition_upstream / guard1_state_pred /
            # co_enabler / 等），其 key 也是 procedure ID，在 multi_instance 展开
            # 后同样需要配对重写。否则 validator 会发现 dep_origins 引用了
            # 已不存在的 base ID（如 PROC-072 而非 PROC-072.1），计为 dangling_deps。
            for origins_field in ("dep_origins", "weak_origins"):
                origins = s3.get(origins_field, {})
                if not origins:
                    continue
                new_origins: dict[str, str] = {}
                seen_origin_keys: set[str] = set()
                for d, origin in origins.items():
                    if d in expanded_ids_by_base:
                        inst_ids = expanded_ids_by_base[d]
                        if my_inst_num <= len(inst_ids):
                            paired_id = inst_ids[my_inst_num - 1]
                            ...
                            paired_id = inst_ids[-1]
                        if paired_id != p["temp_id"] and paired_id not in seen_origin_keys:
                            new_origins[paired_id] = origin
                            seen_origin_keys.add(paired_id)
                    elif d != p["temp_id"] and d not in seen_origin_keys:
                        # 单实例 dep，保留原样
                        new_origins[d] = origin
                        seen_origin_keys.add(d)
                s3[origins_field] = new_origins
